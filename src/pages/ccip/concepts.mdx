...
{/* prettier-ignore */}
<Tooltip description="Finality refers to the state of irreversibility and permanent record of a transaction on the blockchain. If the parameters for finality are properly set, the likelihood of irreversibility is extremely low. For CCIP, source chain finality is the main factor that determines the end-to-end elapsed time for CCIP to send a message from one chain to another." client:visible>**Finality**</Tooltip> refers to the state of irreversibility and permanent record of a transaction on the blockchain. If the parameters
for finality are properly set, the likelihood of irreversibility is extremely low. For CCIP, source chain finality is the
main factor that determines the end-to-end elapsed time for CCIP to send a message from one chain to another.
